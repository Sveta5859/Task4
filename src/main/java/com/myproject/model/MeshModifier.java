...
public class MeshModifier {
    public static void deleteVertex(Model model, int index) {
        if (index<0 || index>=model.getVertices().size()) throw new IndexOutOfBoundsException("Vertex index out of range");
    ...
    public static void deletePolygon(Model model, int index) {
        if (index<0 || index>=model.getPolygons().size()) throw new IndexOutOfBoundsException("Polygon index out of range");
